...
encodeURI(c.uploadName||c.name)+'"');if(!e)a.contentType=c.type||"application/octet-stream",a.data=a.blob||c;else if(d.support.xhrFormDataFileUpload){if(a.postMessage){var g=this._getFormData(a);a.blob?g.push({name:f,value:a.blob}):d.each(a.files,function(b,c){g.push({name:"array"===d.type(a.paramName)&&a.paramName[b]||f,value:c})})}else b._isInstanceOf("FormData",a.formData)?g=a.formData:(g=new FormData,d.each(this._getFormData(a),function(a,b){g.append(b.name,b.value)})),a.blob?g.append(f,a.blob,
c.uploadName||c.name):d.each(a.files,function(c,e){(b._isInstanceOf("File",e)||b._isInstanceOf("Blob",e))&&g.append("array"===d.type(a.paramName)&&a.paramName[c]||f,e,e.uploadName||e.name)});a.data=g}a.blob=null},_initIframeSettings:function(a){var b=d("<a></a>").prop("href",a.url).prop("host");a.dataType="iframe "+(a.dataType||"");a.formData=this._getFormData(a);a.redirect&&b&&b!==location.host&&a.formData.push({name:a.redirectParamName||"redirect",value:a.redirect})},_initDataSettings:function(a){this._isXHRUpload(a)?
...
k.call(e,g,g+("function"===d.type(h)?h(b):h),e.type);b.chunkSize=b.blob.size;b.contentRange="bytes "+g+"-"+(g+b.chunkSize-1)+"/"+f;c._trigger("chunkbeforesend",null,b);c._initXHRData(b);c._initProgressListener(b);m=(!1!==c._trigger("chunksend",null,b)&&d.ajax(b)||c._getXHRPromise(!1,b.context)).done(function(e,h,k){g=c._getUploadedBytes(k)||g+b.chunkSize;p+b.chunkSize-b._progress.loaded&&c._onProgress(d.Event("progress",{lengthComputable:!0,loaded:g-b.uploadedBytes,total:g-b.uploadedBytes}),b);a.uploadedBytes=
b.uploadedBytes=g;b.result=e;b.textStatus=h;b.jqXHR=k;c._trigger("chunkdone",null,b);c._trigger("chunkalways",null,b);g<f?n():l.resolveWith(b.context,[e,h,k])}).fail(function(a,d,e){b.jqXHR=a;b.textStatus=d;b.errorThrown=e;c._trigger("chunkfail",null,b);c._trigger("chunkalways",null,b);l.rejectWith(b.context,[a,d,e])}).always(function(){c._deinitProgressListener(b)})};this._enhancePromise(p);p.abort=function(){return m.abort()};n();return p},_beforeSend:function(a,b){0===this._active&&(this._trigger("start"),
...
{lengthComputable:!0,loaded:f,total:f}),e);g.result=e.result=a;g.textStatus=e.textStatus=b;g.jqXHR=e.jqXHR=c;this._trigger("done",null,e)},_onFail:function(a,b,c,d){var e=d._response;d.recalculateProgress&&(this._progress.loaded-=d._progress.loaded,this._progress.total-=d._progress.total);e.jqXHR=d.jqXHR=a;e.textStatus=d.textStatus=b;e.errorThrown=d.errorThrown=c;this._trigger("fail",null,d)},_onAlways:function(a,b,c,d){this._trigger("always",null,d)},_onSend:function(a,b){b.submit||this._addConvenienceMethods(a,
b);var c=this,e,f,g=c._getAJAXSettings(b),h=function(){c._sending+=1;g._bitrateTimer=new c._BitrateTimer;return e=e||((f||!1===c._trigger("send",d.Event("send",{delegatedEvent:a}),g))&&c._getXHRPromise(!1,g.context,f)||c._chunkedUpload(g)||d.ajax(g)).done(function(a,b,d){c._onDone(a,b,d,g)}).fail(function(a,b,d){c._onFail(a,b,d,g)}).always(function(a,b,d){c._deinitProgressListener(g);c._onAlways(a,b,d,g);--c._sending;--c._active;if(g.limitConcurrentUploads&&g.limitConcurrentUploads>c._sending)for(a=
c._slots.shift();a;){if("pending"===c._getDeferredState(a)){a.resolve();break}a=c._slots.shift()}0===c._active&&c._trigger("stop")})};this._beforeSend(a,g);if(this.options.sequentialUploads||this.options.limitConcurrentUploads&&this.options.limitConcurrentUploads<=this._sending){if(1<this.options.limitConcurrentUploads){var k=d.Deferred();this._slots.push(k);var l=k.then(h)}else l=this._sequence=this._sequence.then(h,h);l.abort=function(){f=[void 0,"abort","abort"];return e?e.abort():(k&&k.rejectWith(g.context,
f),h())};return this._enhancePromise(l)}return h()},_onAdd:function(a,b){var c=this,e=!0,f=d.extend({},this.options,b),g=b.files,h=g.length,k=f.limitMultiFileUploads,l=f.limitMultiFileUploadSize,p=f.limitMultiFileUploadSizeOverhead,m=0,n=this._getParamName(f),t=0;if(!h)return!1;l&&void 0===g[0].size&&(l=void 0);if((f.singleFileUploads||k||l)&&this._isXHRUpload(f))if(f.singleFileUploads||l||!k)if(!f.singleFileUploads&&l){var q=[];var r=[];for(f=0;f<h;f+=1)if(m+=g[f].size+p,f+1===h||m+g[f+1].size+p>
l||k&&f+1-t>=k)q.push(g.slice(t,f+1)),m=n.slice(t,f+1),m.length||(m=n),r.push(m),t=f+1,m=0}else r=n;else for(q=[],r=[],f=0;f<h;f+=k)q.push(g.slice(f,f+k)),m=n.slice(f,f+k),m.length||(m=n),r.push(m);else q=[g],r=[n];b.originalFiles=g;d.each(q||g,function(g,f){var h=d.extend({},b);h.files=q?f:[f];h.paramName=r[g];c._initResponseObject(h);c._initProgressObject(h);c._addConvenienceMethods(a,h);return e=c._trigger("add",d.Event("add",{delegatedEvent:a}),h)});return e},_replaceFileInput:function(a){var b=
a.fileInput,c=b.clone(!0),e=b.is(document.activeElement);a.fileInputClone=c;d("<form></form>").append(c)[0].reset();b.after(c).detach();e&&c.focus();d.cleanData(b.unbind("remove"));this.options.fileInput=this.options.fileInput.map(function(a,d){return d===b[0]?c[0]:d});b[0]===this.element[0]&&(this.element=c)},_handleFileTreeEntry:function(a,b){var c=this,e=d.Deferred(),f=[],g=function(b){b&&!b.entry&&(b.entry=a);e.resolve([b])},h=function(d){c._handleFileTreeEntries(d,b+a.name+"/").done(function(a){e.resolve(a)}).fail(g)},
